// Background.java
import java.awt.image.BufferedImage;
import java.awt.*;
import java.util.ArrayList;
/*
 * Lớp Background điều khiển việc di chuyển của hình nền và mặt đất, phần của hiệu ứng hình ảnh.
 *
 */
public class Background {

    // Hình ảnh của background và mặt đất
    private BufferedImage bg;
    private BufferedImage ground;
    private BufferedImage groundLine = Util.loadBuffImage("assets/ground/groundLine.png");

    //arraylist lưu toạ độ x của các spikes
    private ArrayList<Integer> groundX = new ArrayList<Integer> ();
    private ArrayList<Integer> backgroundX = new ArrayList<Integer>();

    //kích thước của background và ground
    private int bw, bh, gw, gh;

    //toạ độ y
    private int backgroundY = 0;
    private int GroundY = Consts.floor;

    // Vận tốc di chuyển của background và mặt đấ
    private int bv = -2;
    private int gv = -22;

    //màu sắc
    private double blue = 155;
    private double red = 100;
    private double green = 1;


    public Background( BufferedImage bg, BufferedImage ground) {
        this.bg = bg;
        this.ground = ground;
        this.bw = bg.getWidth();
        this.bh = bg.getHeight();
        this.gw = ground.getWidth();
        this.gh = ground.getHeight();

        //toạ do X
        int Xcounter = 0;
        for (int i = 0; i < 5 ; i++) {
            backgroundX.add(Xcounter);
            Xcounter+=bw;
        }

        Xcounter = 0;
        for (int i = 0; i<8; i++) {
            groundX.add(Xcounter);
            Xcounter+=gw;
        }


    }

    //Phương thức di chuyển các tọa độ x của background và mặt đất.
    public void move() {
        for (int i = 0; i < backgroundX.size(); i++) {
            int x = backgroundX.get(i);
            x += bv;
            if (x + bw < 0) {
                backgroundX.set(i, backgroundX.get((backgroundX.size() -1) % backgroundX.size() ) + bw);
            } else {
                backgroundX.set(i, x);
            }
        }

        for (int i = 0; i < groundX.size(); i++) {
            int x = groundX.get(i);
            x += gv;
            groundX.set(i, x);
            if (x + gw < 0) {
                groundX.set(i, groundX.get( ((i + groundX.size() -1 ) % (groundX.size())) ) + gw-20);
            }
        }

    }

    //Phương thức vẽ background và mặt đất.
    public void draw(Graphics g, int offsetY) {

        for (Integer x : backgroundX) {
            g.drawImage(bg, x, backgroundY, bw, bh, null);
        }

        for (Integer x : groundX) {
            g.drawImage(ground, x, GroundY + offsetY, gw, gh, null);
        }

        g.setColor(new Color((int)red, (int)green, (int)blue, 80));
        g.fillRect(0, 0, Consts.SCREEN_WIDTH, Consts.SCREEN_HEIGHT);

        int tmp = 250;
        g.drawImage(groundLine, 0, GroundY + offsetY, Consts.SCREEN_WIDTH, 5, null);

    }

    //Phương thức vẽ background cho màn hình menu
    public void mainMenuDraw(Graphics g) {
        for (Integer x : backgroundX) {
            g.drawImage(bg, x, backgroundY, bw, bh, null);
        }

        for (Integer x : groundX) {
            g.drawImage(ground, x, GroundY - 90, gw, gh, null);
        }

        g.setColor(new Color(0, 0, 255, 120));
        g.fillRect(0, 0, Consts.SCREEN_WIDTH, Consts.SCREEN_HEIGHT);


    }
}
